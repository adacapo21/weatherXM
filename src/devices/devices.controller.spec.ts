...
describe('DevicesController', () => {
      ...
      providers: [
        {
          ...
          useValue: {
            ...
            updateDeviceById: jest
                .fn()
                .mockImplementation((device: WeatherDeviceDto) =>
  ...
  describe('findOne Device', () => {
    it('should get a Device by its id', async () => {
      await expect(devicesController.findOne(device.id)).resolves.toEqual(device);
    });
  });
  
  describe('Update Device By Id', () => {
    it('should update a Device by its id', async () => {
      await expect(devicesController.update(device.id, updatedDevice))
